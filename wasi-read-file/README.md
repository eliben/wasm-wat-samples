...
which directories are available. In this example, we assume only one directory
is mapped - `/`; therefore, the WASM code assumes it can find it at the first
file descriptor available for mapping - 3.

After compiling the WAT file to `readfile.wat`, this sample can be tested
